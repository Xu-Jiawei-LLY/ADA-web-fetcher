...
function matchesChina(text) {
  const lower = text.toLowerCase();
  const pharmaMatch = CHINESE_PHARMA_KEYWORDS.some(kw => lower.includes(kw));
  if (pharmaMatch) return true;
  // Use word boundary matching for population keywords to avoid false positives
  return CHINA_POPULATION_KEYWORDS.some(kw => {
...
function identifyCompany(text) {
  ...
  const companyMap = {
    ...
    'CSPC': ['cspc'],
    'Gan & Lee': ['gan & lee', 'gan and lee'],
    'Tonghua Dongbao': ['tonghua dongbao'],
    'Salubris': ['salubris'],
    'Beigene': ['beigene'],
    'Zelgen': ['zelgen'],
    'Sciwind': ['sciwind'],
    ...
    'I-Mab': ['i-mab'],
    'Kelun': ['kelun'],
    'Bright Gene (BGM)': ['bgm0504', 'bright gene'],
    'Dong-A': ['dong-a']
  };
  for (const [name, keywords] of Object.entries(companyMap)) {
    if (keywords.some(kw => lower.includes(kw))) found.push(name);
  }
  if (found.length === 0 && matchesChina(text)) {
    if (CHINA_POPULATION_KEYWORDS.some(kw => lower.includes(kw))) {
      found.push('Chinese Population Study');
    }
  }
  return found.length > 0 ? found.join(', ') : 'Unknown Chinese Entity';
}
...
function filterArticles(articles) {
  console.log('[6/6] Filtering for T2D + China/Chinese pharma...');
  const matched = [];
  const titleOnlyMatched = [];

  for (const art of articles) {
    const combinedText = `${art.title} ${art.authors}`;
    const isT2D = matchesT2D(combinedText);
    const isChina = matchesChina(combinedText);
  ...
  console.log(`   Direct matches (T2D + China in title/authors): ${matched.length}`);
...
async function checkAbstracts(cdp, articles) {
  ...
  let checked = 0;
  const toCheck = articles.slice(0, 100);
  for (const art of toCheck) {
    ...
    if (matchesChina(abstractText)) {
      art.abstractSnippet = abstractText.substring(0, 300);
...
<head>
  <meta charset="UTF-8">
  <title>ADA 2026 - T2D China/Chinese Pharma Report</title>
  <style>
...
<body>
  <h1>ADA 86th Scientific Sessions (2026)</h1>
  <p class="subtitle">Type 2 Diabetes & China/Chinese Pharma — Categorized by Compound | <a href="${ISSUE_URL}" target="_blank">Source</a> | Generated: ${new Date().toISOString().split('T')[0]}</p>
...
async function main() {
  ...
  console.log('=== ADA 2025 T2D/China Scraper ===\n');
  ...
  const articles = await expandAndCollectArticles(cdp);
  ...
  let hba1cData = [];
  const existingCsv = path.join(outputDir, 'hba1c_data.csv');
  ...
  const rawPath = path.join(outputDir, 'raw_results.json');
